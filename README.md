...
- What is the runtime complexity to access an array, add or remove from the front, and add or remove from the back?
  ...
  Access = O(1)
  Add/Rem(Back) = O(1)
  Add/Rem(Front) = O(n) where n is distance from last index

- What is the worse case scenario if you try to extend the storage size of a dynamic array?

  The worst case is exceeding the process memory limit and getting killed by the OS. Next would be needing to find a new location in memory and having to relocate the entire array

Explain how a blockchain is structured. What are the blocks, what is the chain? How is the data organized?

A blockchain is essentially is ordered list(the chain) of objects that are hard to create(the blocks) that require the previous block in order to be created.

Explain how proof of work functions. How does it operate. How does this protect the chain from attack. What kind of attack is possible?

Proof of work is a solution to an arbitrarily difficult problem. The problem is best when it can't be predictably solved. This protects the chain because if an attacker want to change one block they
must also change every following block. This means they must compute faster than everyone else mining the chain put together.
